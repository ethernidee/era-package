...
////_Scope_ *ERMScope=0;
struct _ERMScript_{
  int   Used;  // 2-не используется в карте/1-загружен/0-no
//  char  FileName[63]; // имя файла
  int   Size; // размер в байтах загруженного файла
...
struct _MonNames_{
// 0-Имя один, 1-Имя много, 2-Специальность
...
char *GetERMtext(Mes *Mp,int ind,int staticonly){
  ...
  char *ret="";
  int vv;
  if(Mp->VarI[ind].Num!=0){ // переменная
    Apply(&vv,4,Mp,ind);
    if(staticonly){
...
int GetERMScripts(int ind)
{
  ...
  if(WoGType){ Message("Не найти ERM скрипт.",1); }
...
static int _LoadERMScript(int Number,char *name,char *path)
{
  ...
  if(Service_GetFileAttributes(&TxtName[3])==-1) RETURN(-1) // нет такого файла
  if(ErmTXTScript.fl!=0) UnloadTXT(&ErmTXTScript);
  if(Number==0){
    if(WoG==0){ // не WoG
      if(WoGType){ Message("Так как карта не WoG, то не будет WoGифицирована.",1); }
      else{ Message("Since the map is not a WoG type, it will not be WoGified.",1); }
      RETURN(1)
    }
  }
  if(LoadTXT(TxtName,&ErmTXTScript)){
    if(WoGType){ Message("Не загрузить скрипт.",1); }
...
//   for(i=0;i<63;i++) ERMScript[Number].FileName[i]=TxtName[i];
  ...
  if(ERMScript[Number].Size<=0){
    if(WoGType){ Message("Размер файла ERM скрипта некоректен.",1); }
...
//  TxtName[StrLen(TxtName)-1]='T';
  ...
  if(ErmTXTScript.fl!=0) UnloadTXT(&ErmTXTScript);
  if(LoadTXT(TxtName,&ErmTXTScript)){
    if(WoGType){ Message("Не загрузить файл строк для скрипта.",1); }
...
//  ReadStrINI(Load2Path,MAX_PATH-1,"","Alternate_Script_Location","WoGification");
  ...
  ERMScript[Number].Used=0;
  ret=-1;
  if(Load2Path[0]!=0){ // есть альтернативный путь
    ret=_LoadERMScript(Number,name,Load2Path);
  }
  if(ret==-1){ // там нет файла (или пути)
    ret=_LoadERMScript(Number,name,LCT_s2);
  }
  if(ret==-1) RETURN(0) // нет такого файла вообще
  if(ret==1) RETURN(1) // ошибка случилась
  ret=-1;
  if(Load2Path[0]!=0){ // есть альтернативный путь
...
*/
    ...
    if(LoadFile16k(&TxtName[3],"rb")==0) RETURN(0) // не может загрузить TXT - нету его
    if(ErmTxt.fl!=0) UnloadTXT(&ErmTxt);
    if(LoadTXT(TxtName,&ErmTxt)){
      if(WoGType){ Message("Не загрузить ERM файл.",1); }
      else{ Message("Cannot load ERM file.",1); }
      RETURN(1) // что-то не так
    }
    YesERMFile=1;
...
int ReParseVal(VarNum *vn)
{
  #include "templ.h"
  int vi=vn->Num;
  if(vn->IType!=0){// если есть индексирование
    switch(vn->IType){
...
//      ERMVarX[15]=j; // цикловая переменная
        // 3.58 return values
        for(i=0;i<Num;i++){
          if(Mp->VarI[i].Check!=1) continue; // не ?
          ERMVarX[i]=Mp->n[i];
        ...
        for(i=0;i<Num;i++){
          if(Mp->VarI[i].Check!=2) continue; // не =
//          Mp->VarI[i].Check=0;
//          Apply(&ERMVarX[i],4,Mp,(char)i);
//          Mp->VarI[i].Check=2;
...
//        ind=vnp->Num;
      ...
      }else{ MError("\"!!VR:С\"-not a v type variable."); RETURN(0) }
      break;
    case 'U': // search for
     { char *s,*d;
      if(Num!=1){ MError("\"!!VR\"-wrong number of parameters."); RETURN(0) }
...
static struct _ERM_Object_{
  ...
  Word  HintVar;     // переменная подсказки 0-нет,1...
...
struct _ERM_Addition_{
  ...
  {'AM',ERM_MonAtr,0},
  {'NU',ERM_Universal,0},
  {'RV',ERM_Variable,1}, // скопировать переменную VRi
  {'MT',ERM_Timer,1},    // скопировать переменную TMi
  {'BO',ERM_SetObject,2},   // 2-x/y/l в MixedPos
  {'WO',ERM_Owner,0},
  {'NM',ERM_Mine,2},
  {'CS',ERM_SetScoolar,2},   // 2-x/y/l в MixedPos
  {'HC',ERM_SetChest,2},     // 2-x/y/l в MixedPos
  {'TW',ERM_SetWTomb,2},     // 2-x/y/l в MixedPos
  {'TK',ERM_SetKTree,2},     // 2-x/y/l в MixedPos
  {'RF',ERM_SetFire,2},      // 2-x/y/l в MixedPos
  {'NL',ERM_SetLean,2},      // 2-x/y/l в MixedPos
  {'TS',ERM_SetStone,2},     // 2-x/y/l в MixedPos
  {'GW',ERM_SetWagon,2},     // 2-x/y/l в MixedPos
  {'KS',ERM_SetSkelet,2},    // 2-x/y/l в MixedPos
  {'PS',ERM_SetSpring,2},    // 2-x/y/l в MixedPos
  {'MW',ERM_SetWMill,2},     // 2-x/y/l в MixedPos
  {'WS',ERM_SetSwan,2},      // 2-x/y/l в MixedPos
  {'TM',ERM_SetMonolit,2},   // 2-x/y/l в MixedPos
  {'DG',ERM_SetGarden,2},    // 2-x/y/l в MixedPos
  {'LM',ERM_SetMill,2},      // 2-x/y/l в MixedPos
  {'WD',ERM_SetDwelling,2},  // 2-x/y/l в MixedPos
  {'HW',ERM_SetWHat,2},      // 2-x/y/l в MixedPos
  {'YS',ERM_Shipyard,2},     // 2-x/y/l в MixedPos
  {'RG',ERM_Garrison,2},     // 2-x/y/l в MixedPos
  {'RS',ERM_Shrine,2},       // 2-x/y/l в MixedPos
  {'GS',ERM_Sign,2},         // 2-x/y/l в MixedPos
  {'RU',ERM_Univer,2},       // 2-x/y/l в MixedPos
  {'CM',ERM_Macro,1},        // скопировать переменную VRi
  {'UF',ERM_Function,3},     // скопировать значение M.n[0]
  {'OD',ERM_Do,4},           // значение M.n[0], цикл M.n[1],M.n[2],M.n[3]
  {'AC',ERM_Castle,2},       // 2-x/y/l в MixedPos
  {'RT',ERM_Terrain,2},      // 2-x/y/l в MixedPos
  {'OH',ERM_SetHero,3},      // скопировать значение M.n[0]
  {'OP',ERM_Position,2},     // 2-x/y/l в MixedPos
  {'AB',ERM_Battle,0},
...
//  {(Word)'BM',ERM_BMonster,0},
  ...
  {'TH',ERM_HintType,5},      // две переменные v1/v2
  {'MP',ERM_Pyramid,2},       // 2-x/y/l в MixedPos
  {'WM',ERM_WMon,1},          // скопировать переменную MWi
  {'MC',ERM_MouseClick,0},
  {'MM',ERM_MouseMove,0},
  {'PM',ERM_MP3,0},
  {'IA',ERM_AIRun,0},
...
// 3.58
  ...
  {'AE',ERM_AICrExp,2},
  {'BC',ERM_SetCrBank,2},  // 2-x/y/l в MixedPos
  {'PI',ERM_NetworkService,3},      // скопировать значение M.n[0]
// 3.59
  {'LT',ERM_TL,0}, 
  {'LD',ERM_Dlg,1},          // скопировать переменную VRi
  {'SS',ERM_Spell,1}, 
  {'DL',ERM_LODs,0}, 
  {'fi',0,0},
  {'le',0,0},
...
//            if(ivf!=0){ Error(); goto _over; }
            ...
            if(immed){
              if(mvi->IType!=0){// если есть индексирование
                *n=ERMVar[vi-1];
                vi=*n;
              }
              switch(vf){
...
//            if((vi<-10)||(vi>10000)){ MError("\"GetNum\"-wrong variable index (-10...10000)."); goto _over; }
            ...
            if(set){
              ...
              if(immed){
                if(ivf!=0){// если есть индексирование
                  switch(ivf){
                    case 2: *n=ERMVar[vi-1]; break; // f...t
                    case 3: *n=ERMVar2[vi-1]; break; // v
                    case 4: *n=ERMVarH[ERMW][vi-1]; break; // w
...
int AddCommand(_ToDo_ *sp,int b,int e)
{
  ...
  l=PERM->l;
  s=&PERM->s[l];
  if(PERM->p==0){ // нет ни одной постинструкции
    *s++='!'; *s++='@'; // добавим ограничитель
    *s++='Z'; *s++='V'; *s++='S'; *s++='E';
    PERM->p=l+2;
    l+=6;
  }
...
////        }
    ...
    if(j==0){
      if(Efl[0][0][0].Type!=0) RETURN(0) // если есть & часть и все условия прошли
    }
ToOr:; // AND не прошел
  }
  RETURN(1) // и OR не было
}
...
//    if(CheckFlags(M.efl,M.dfl)) continue;
    if(Cmd=='Z'){
      ...
      if(PostInstr==0){ // не дублировать постинструкции
...
// восстанавливает настройки для карт со скриптами
...
{
  #include "templ.h"
  for(int i=200;i<PL_WONUM;i++){ // сбросим все новые опции
    PL_WoGOptions[0][i]=0;
  }
  PL_CrExpStyle=0; 
//  PL_NoNPC=0; ResetNPC();
...
//  Hn=0x9C;  // количество героев героев
// 3.58
//  MS=(_Main_ *)Esi_;
  MS=Main;
...
//  SetMouseCursor(1,1);
  ...
  Post=0;
  if(nL==0) ResetWoG357(); // настройки (копирует установленные)
  ...
  for(GEp_=GEp0;GEp_<=GEp1;GEp_++){
    ...
    ScriptIndex4ERM=0;
    if(GEp_==GEp1){ // этого сообщения уже нет
      // проверим на установку опции ВоГификации (5) в командах 
      if((nL==0)&&(PL_ApplyWoG>0)&&(NumOfScripts==0)&&(FirstMesFlag==0)){
        // если Новая и Надо как-то ВоГифицировать и скрипты не были подгружены и первый раз
        if(CountERMScripts()==0) LoadERMTXT(1);
      ...
      if((nL==0)&&(NumOfScripts==0)&&(FirstMesFlag==0)){
        // если новая игра, не WoG и первый раз входим
        if(FirstFlag>1){ // если были найдены ERM команды в самой карте
//            if((PL_DisableWogifyWasSet!=0)&&(PL_ApplyWoG==0)){
//              ResetWoG358(); // если есть скрипты, И ПРИРАВНЕН к старым, то 358
//            }
          if(PL_ExpoWasSet!=0){ // опции для 3.58 должны быть - ничего не меяем в установках
        ...
        }else{ // в карте нет скриптов
          // сообщаем, что не вогифицируется
          if((PL_ApplyWoG==0)) Message(ITxt(199,0,&Strings),1);
        }
      }
      if(YesERMFile && ERMFile2Process){ // нашли файл с ERM
        M.m.s=ITxt(0,0,&ErmTxt);
        M.m.l=StrLen(ITxt(0,0,&ErmTxt));
        M.i=0;
        GEp_--; // откатим назад
        ERMFile2Process=0; // отменим повторный вход
      }else if(NumOfScripts!=0){
        // нашли скрипты ВоГификации (WoG файл или приравнен к нему)
        if(FirstMesFlag==0){ // входим первый раз
          if(YesERMFile){ // есть файл *.ERM
            if(nL){ // загрузка
              if(LoadVersion4Scripts>9){ // 3.57m+ ненужные скрипты и так не записываются
              }else{ // 3.57  были все скрипты записаны - пропустим
                ResetWoG356();
                break;
              }
            }else{ // новая
          ...
          }else if(FirstFlag>1){ // есть внутренние скрипты
            if(nL){ // загрузка
              if(LoadVersion4Scripts>9){ // 3.57m+ ненужные скрипты и так не записываются
              }else{ // 3.57  были все скрипты записаны - пропустим
                ResetWoG356();
          ...
          }else{ // без скриптов совсем - сделаем ее типа ВоГ, т.к. вогифицируем
        ...
        Post=1;
        if(nL &&(PERM->p!=0)){ // обработаем постинструкции
          M.m.s=&PERM->s[PERM->p]; M.m.l=PERM->l-PERM->p; M.i=0;
        }else break; // все обработали
      }
    }else{
      M.m.s=GEp_->Mes.m.s; M.m.l=GEp_->Mes.m.l; M.i=0;
    }
    if(CheckMine(&M)){ // проверяем заголовок
      if(FirstFlag==0){ // вход первый раз
        if(GEp0==0){ // если вообще нет глобальных сообщений
      ...
      ++FirstFlag;
      if(Post && nL){
        // Сюда должны попасть только один раз
        // предпологаем, что первая пост инструкция - !#IF:A..........
      }
      do{ // найти генератор действия
        M.c[0]='!';
        // 3.58 correction to skip ^ in comments
        if(SkipUntil2(&M)) break; // нет генератора - к след событию
        if(M.i>(M.m.l+5)) break; //нет места для команды
//    GEr.LastERM(sp->Self.s); // 3.58
...
//              cp->Par[0]=M.VarI[0]; cp->ParSet=1;
              break;
            case 'MC': // VC 'CM': // CM; клик мауса (0...5)
              M.c[0]=';';
              Ind=M.i;
              ...
              break;
            case 'MM': // MM; перем мауса (0-битва,1-город)
              M.c[0]=';';
              Ind=M.i;
              ...
              break;
            case 'EA': // VC 'AE': // AE; одевание артифакта (0-снять,1-одеть)
              M.c[0]=';';
              Ind=M.i;
...
//              cp->Par[0]=M.VarI[0]; cp->ParSet=1;
              break;
            case 'PM': // VC 'MP': // MP; переключение MP3
              M.c[0]=';';
              Ind=M.i;
            ...
            case 'GM': // VC 'MG': // MG; magic casting on adv. map
              ...
              break;
            case 'NS': // VC 'SN': // SN; переключение Sound
              M.c[0]=';';
              Ind=M.i;
...
////          ERMScope=FindScope(cp->Type);
        ...
        }else if((M.m.s[M.i]=='!')||(M.m.s[M.i]=='#')){ // событие-приемник или инструкция
...
//                ToDoPo2=PosMixed(M.n[0],M.n[1],M.n[2]);
              ...
              break;
            case 0x4548: // HE#: или HE#1/#2/#3: или HE-1: или HEv:
              M.c[0]=':';
              Ind=M.i;
              switch(GetNumAutoFl(&M)){
                case 1:
...
//          dp->Pointer[1]=ToDoPo2;
          ...
          if(InstrFlag){
            if((nL==0)|| Post){ // не выполнять обычные инструкции при загрузке
              if(Post){ // не надо флаги проверять
                // если записано, значит было выполнено и должно быть
                // повторено безусловно
                ProcessCmd(dp,!nL,1); // если новая игра, а не загрузка
              }else{
                if(nL==0){
                  if(CheckFlags(M.Efl)==0){ // условное выполнение
                    ProcessCmd(dp,1,0); // если новая игра, а не загрузка
                  }
                }
              }
            }
...
//  char   OldString[10][512];
  ...
  cp=Heap;
  if(ERMEnabled==0) RETURNV // ERM не включен
  while(cp->Event!=0){
...
/*
            if(CheckFlags(cp->ToDo[i].Efl)) continue; // условное выполнение
...
void _PostInstrCall(void){
  #include "templ.h"
  if(nL==0){ // если только новая игра
    Dword oldpo=pointer;
    pointer=30370;
    ProcessERM();
    pointer=oldpo;
...
*/
  if(Mp->OType==0x22){ // герой
    if(ThisSquare){ // герой и стоит здесь
      ind=Mp->SetUp;
      hv=ERM_Hero[ind].HintVar;
      if(hv==0) RETURN(0) // не задано
...
//      ret=CheckObjHint((_MapItem_ *)ind,HW_Buf);
    ...
    if((mi->OType==63)&&(mi->OSType==0)){ // пирамида
...
void ClearScriptFile(void)
{
  ...
  for(i=0;i<SCRIPTSNUM;i++){
    if(ERMScript[i].Used!=0){ // освободим память
      Free(ERMBody[i]);
    }
    ERMScript[i].Used=0;
