    ...
    case 'N': // create new dialog from template
      ...
      if(dlg==NULL){ // not found - try to load
        ...
        if(Mp->VarI[0].Num!=0){ // переменная
          Apply(&vv,4,Mp,0);
          if((vv<-10)||(vv==0)){ MError("\"!!DL:N\"-wrong z var index (-10...-1,1...1000+)."); RETURN(0) }
          if(vv>1000)   FileName=ERM2String(StringSet::GetText(vv),1,0);
          else if(vv>0) FileName=ERM2String(ERMString[vv-1],1,0);
          else          FileName=ERM2String(ERMLString[-vv-1],1,0);
...
//      break;
    case 'E':{ // enable/disable item
      // Eid/ENdis
      if(Num<2){ MError("\"DL:E\"- wrong syntax."); RETURN(0) }
      if(dlg==NULL) break; // not found
    ...
    case 'H':{ // set item hint
      ...
      if(Mp->VarI[1].Num!=0){ // переменная
        Apply(&vv,4,Mp,1);
        if(vv<1){ MError("\"!!DL:H\"-wrong z var index (1...1000+)."); RETURN(0) }
        if(vv>1000)   hint=StringSet::GetText(vv);
        else if(vv>0) hint=ERMString[vv-1];
      }else{ MError("\"!!DL:H\"-cannot use direct ERM text (z vars only)"); RETURN(0) }
      dlg->AddHint(id,hint);
      break;}
    case 'A':{ // send command to control
      ...
      if(Mp->VarI[2].Type==7){ // z var
        if((vv<-10)||(vv==0)){ MError("\"!!DL:A\"-wrong z var index (-10...-1,1...1000+)."); RETURN(0) }
        if(vv>1000)   par=(Dword)ERM2String(StringSet::GetText(vv),1,0);
        else if(vv>0) par=(Dword)ERM2String(ERMString[vv-1],1,0);
        else          par=(Dword)ERM2String(ERMLString[-vv-1],1,0);
      }else{
        par=vv;
      }
//__asm int 3
//      dlg->SendCmd2Item(cmd,id,par);
      ...
      ERMFlags[0]=1;
      break;}
    default: MError("\"!!DL\"-wrong command."); RETURN(0)
  }
  RETURN(1)
}
...
static Dword ShowMapSpellDlg_VTable[15]={
  0x4917E0, // Dctor
  0x5FF0A0, // Activate
  0x5FF220, // Disactivate
  0x405610,  
