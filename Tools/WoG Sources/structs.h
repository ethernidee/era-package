...
struct _Cmd_{
  ...
  Dword  Event;        // адрес структуры иниц. евента
...
struct _PostERM_{
  long   f;     // полный размер строки (заполняется при выделении)
  long   l;     // длина полного сообщения
  long   p;     // смещение до пост обработки
  _Mes_ *owner; // указатель на сообщение - хозяин (куда добавляем)
  char   s[10]; // не важно сколько - размер в начале
};
...
struct _PlayerSetup_{
  ...
  char _u3[4];     //  +60
//  int   TopHeroInd; // +64 = индекс среди 8 героев, который верхний
