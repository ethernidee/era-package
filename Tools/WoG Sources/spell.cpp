...
static char *GetText(int ind,int tp){
  ...
  int vv=ZVars[ind][tp];
  if(vv){
    if(vv>1000)   po=StringSet::GetText(vv);
    else if(vv>0) po=ERMString[vv-1];
  }
  RETURN(po)
}
...
//  int    Cost[4];         // +20h cost mana per skill level
    case 'C': // C#/$
      if(Num<2){ MError("\"SS:C\"- wrong syntax."); RETURN(0) }
      Apply(&vv,4,Mp,0);
...
//  int    Effect[4];       // +34h effect per skill level
    case 'E': // E#/$
      if(Num<2){ MError("\"SS:E\"- wrong syntax."); RETURN(0) }
      Apply(&vv,4,Mp,0);
...
//  int    Chance2GetVar[9];// +44h chance per class
    case 'H': // H#/$
      if(Num<2){ MError("\"SS:H\"- wrong syntax."); RETURN(0) }
      Apply(&vv,4,Mp,0);
...
//  int    AIValue[4];      // +68h 
    case 'I': // I#/$
      if(Num<2){ MError("\"SS:I\"- wrong syntax."); RETURN(0) }
      Apply(&vv,4,Mp,0);
...
//  char  *Descript[4];     // +78h
    case 'D': //D#/$
      if(Num<2){ MError("\"SS:D\"- wrong syntax."); RETURN(0) }
      Apply(&vv,4,Mp,0);
      ...
      break;
    default: MError("\"!!SS\"-wrong command."); RETURN(0)
  }
  RETURN(1)
}
