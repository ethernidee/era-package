...
//        }
        ...
        int State[16]; for(i=0;i<16;i++) State[i]=0;
        ...
        for(i=0;i<16;i++){ if(State[i]==1) SI_Cr->SetSubArt(i); }
        SI_ArtButton=1;
        RETURN(1)
      }else{
...
//      CrExpMod::Copy(CrExpMod::Get(Type),CrExpMod::Get(v));
    ...
    case 'R':{ // add artifact R$art/$option
      if(Num<2){ MError("\"!!EA:R\"-wrong number of parameters."); RETURN(0) }
    ...
    case 'E':{
      ...
      if(Num<4){ MError("\"!!EA:E\"-wrong number of parameters."); RETURN(0) }
      Type=-Type-1;
      ...
      break;}
    case 'F':
      if(Num<2){ MError("\"!!EA:F\"-wrong number of parameters."); RETURN(0) }
      if(Num<3){
    ...
    case 'H': // Human Expo Multiplier
      ...
      break;
    default: MError("\"!!EA:H\"-wrong command."); RETURN(0)
  }
  RETURN(1)
}
