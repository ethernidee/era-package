...
int LoadERSfile(int ind){
  #include "templ.h"
  int ret=-1;
  char  TxtName[1024];
  char *name=GetScriptName(ind+2); 
  if(Load2Path[0]!=0){ // есть альтернативный путь
    ret=0;
    sprintf(TxtName,"%s%s.ers",Load2Path,name);
    if(Service_GetFileAttributes(&TxtName[3])==-1) ret=-1; // нет такого файла
  }
  if(ret==-1){ // там нет файла (или пути)
    ret=0;
    sprintf(TxtName,"%s%s.ers","..\\DATA\\S\\",name);
    if(Service_GetFileAttributes(&TxtName[3])==-1) ret=-1; // нет такого файла
  }
  if(ret==-1) RETURN(-1)
...
int LoadSetupParam(void)
{
  ...
  ReadStrINI(WoGFilePath,MAX_PATH-1,".\\","Options_File_Path","WoGification");
  ReadStrINI(WoGFileName,MAX_PATH-1,"WoGSetupEx.dat","Options_File_Name","WoGification");
  ReadStrINI(Load2Path,MAX_PATH-1,"","Alternate_Script_Location","WoGification");
  for(i=0;i<1100;i++){ LoadERSfile(i); }
  ProcessAll();
  InitWoGSetup();
  for(i=0;i<PL_WONUM;i++) PL_WoGOptions[1][i]=PL_WoGOptions[0][i];
  RETURN(0)
