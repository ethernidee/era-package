...
void _Error(int File,int Line)
{
  #include "templ.h"
  if(File==0){
    if(WoGType){ Zsprintf2(&TermStruct,"Ошибка синтаксиса ERM. Причина не указана.",0,0); }
    else{ Zsprintf2(&TermStruct,"ERM syntax Error. No reason.",0,0); }
  }else{
    if(WoGType){ Zsprintf2(&TermStruct,"Ошибка синтаксиса ERM. \n\tФайл: %s\n\tСтрока : %i",(Dword)FileList[File],(Dword)Line); }
...
void _MError(int File,int Line,char *Txt)
{
  ...
  if(Txt==0){
    ...
    if(WoGType){ Zsprintf3(&TermStruct,"Ошибка синтаксиса ERM.\n\tФайл: %s\n\tСтрока: %i\n\tПричина:\n\t%s\n\nСохранить дамп ERM переменных в WOGERMLOG.TXT (длит. процедура)?",(Dword)FileList[File],(Dword)Line,(Dword)Txt); }
...
int WeekOf(void)
{
  ...
  if(NextWeekMess){ // установим сообщение в переменную
...
void CastSpell(int Spell,int heroMONSTER,int Pos,int HSkill,int MSkill)
{
  ...
  CS_MSkill=MSkill;
  __asm{
    push   CS_HSkill // обычно - количество раундов
    push   CS_MSkill
    push   -1
