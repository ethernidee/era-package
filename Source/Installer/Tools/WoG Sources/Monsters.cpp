...
//  if(WoG==0) return 0;
  ...
  switch(Cmd){
    ...
    case 'L': // Уровень L#/$ (0...6[7],-1 = запрещен)
      ...
      CrIsChanged(k);
      break;
    case 'C': // Цена C#/#/$ по номерам ресурсов (0...7)
      if(Num<3) RETURN(0)
...
void __stdcall LeaveCreature(int,int,int,int,int,int,int,int,int,int)
{
  __asm pusha
...
//  __asm ret 10
//  asm popa
//  asm jmp dword ptr [OriginalCallPointer]
}
...
//      }
    ...
    case 'V':{ // 3.59 V$stack/$varind/$varval
      if(Num<3){ MError("\"!!BG:V\"-wrong syntax."); RETURN(0) }
      int ind=0,st=0;
    ...
    case 'C':{ // 3.59 C or C$stack - clean up all or stack vars
      ...
      break;}
    default: MError("\"!!BG:\"-wrong command."); RETURN(0)
  }
  RETURN(1)
}
...
static Byte *MR_Mon;
int __stdcall MagicResist(int Type)
{
  _ECX(MR_Dam);
  _EDX(MR_Spell);
...
float __stdcall MagicDwarfResistance3(int p2,int p1)
{
  __asm{
    mov    MR_Mon,edi
    mov    eax,[edi+0x34]
...
int ERM_MonRes(char Cmd,int/*Num*/,_ToDo_* /*sp*/,Mes *Mp) //!!MR
{
  ...
  switch(Cmd){
    ...
    default: MError("\"!!MR:\"-wrong command."); RETURN(0)
  }
  RETURN(1)
}
...
//  asm{mov eax,0x642224; mov dword ptr [eax],1; mov dword ptr [eax+4],1; mov dword ptr [eax+8],1; mov dword ptr [eax+12],1 }
  ...
  if(herMON){ // monster
    ...
    CastSpell(10,1,Pos,3,3);
    *(int *)&bm[0x132B8]=v;
    *(int *)&bm[0x132BC]=v2;
  }else{ // hero
    v=*(int *)&bm[0x132C0]; *(int *)&bm[0x132C0]=Num;
    CastSpell(10,0,Pos,3,-1);
    *(int *)&bm[0x132C0]=v;
  }
  *pqb=i;
  __asm{
...
int LandMine(int herMON,int Num,int Pos,int Draw)
{
  ...
  if(herMON){ // monster
    v= *(int *)&bm[0x132B8]; *(int *)&bm[0x132B8]=Num/21;
    v2=*(int *)&bm[0x132BC]; *(int *)&bm[0x132BC]=Num%21;
    CastSpell(11,1,Pos,3,3);
    *(int *)&bm[0x132B8]=v;
    *(int *)&bm[0x132BC]=v2;
  }else{ // hero
    v=*(int *)&bm[0x132C0]; *(int *)&bm[0x132C0]=Num;
    CastSpell(11,0,Pos,3,-1);
    *(int *)&bm[0x132C0]=v;
  }
  *pqb=i;
  __asm{
...
int ERM_BRound(char Cmd,int Num,_ToDo_*sp,Mes *Mp) //!!BM
{
  ...
  if((mn<0)||(mn>41)){
    MError("\"!!BM:\"-monster index is incorrect (0...41).");
    RETURN(0)
  }
  __asm{
    mov    eax,0x699420
    ...
    case 'U': // 3.58 U#/...
      ...
      switch(i){
        ...
        case 4: // U4/$ - тип спела для каста
          Apply((int *)&mon[0x4E0],4,Mp,1);
          break;   
        case 5: // U5/$ - номер стэка - копии данного (-1 = нет)
          Apply((int *)&mon[0x28],4,Mp,1); 
          break;
        default:
          MError("\"!!BM:U\"-wrong syntax."); RETURN(0)
      }
      break;
    case 'G':
      if(Num<3){ MError("\"!!BM:G\"-wrong syntax."); RETURN(0) }
      if(Apply(&Spell,4,Mp,0)){ MError("\"!!BM:G\"-par 1 maybe set only."); RETURN(0) }
...
//          mov    word ptr [eax+4],0x0069
      ...
      break;
    case 'Q': // Q#/pos/redraw
      if(Num<3){ MError("\"!!BM:Q\"-wrong syntax."); RETURN(0) }
      if(Apply(&val,4,Mp,0)) break;
      ...
      switch(val){
        ...
        default:
          MError("\"!!BM:Q\"-wrong syntax."); RETURN(0)
      }
      break;
    case 'V': //V# -animate magic # 3.58
      ...
      break;
    default: MError("\"!!BM:\"-wrong command."); RETURN(0)
  }
  RETURN(1)
}
...
int ERM_BHero(char Cmd,int Num,_ToDo_*sp,Mes *Mp)
{
  ...
  switch(Cmd){
    ...
    case 'C': // Cspell/pos/Hskill/Check4TargetMonster
      if(Num<4){ MError("\"!!BH:C\"-wrong syntax."); RETURN(0) }
      ...
      CastSpell(Spell,0,Pos,Skill,-1);
      *(int *)&bm[0x132C0]=v;
      break;
    case 'Q': // Q#/pos/redraw
      if(Num<3){ MError("\"!!BH:Q\"-wrong syntax."); RETURN(0) }
      ...
      switch(v){
        ...
        default: MError("\"!!BM:Q\"-wrong syntax."); RETURN(0)
      }
      break;
    default:
      MError("\"!!BH:\"-wrong command.");
      RETURN(0)
  }
  RETURN(1)
}
int ERM_BUniversal(char Cmd,int Num,_ToDo_*,Mes *Mp)
{
  ...
  switch(Cmd){
    case 'M': // добавить сообщение
     {
      int sind;
      char *txtp;
      if(Num!=1){ MError("\"!!BU:M\"-wrong syntax."); RETURN(0) }
      if(Mp->VarI[0].Type!=7){ MError("\"!!BU:\"- not a Z variable."); RETURN(0) }
      sind=GetVarVal(&Mp->VarI[0]);
      if((sind<-10)||(sind==0)||(sind>1000)){ MError("\"!!BU:\"-wrong z (destination) index (-10...-1,1...1000)."); RETURN(0) }
    ...
    case 'E': // Epos/?v номер монстра в позиции
      if(Num<2){ MError("\"!!BU:E\"-wrong syntax."); RETURN(0) }
      if(Apply(&v,4,Mp,0)) break;
      bh=(char *)GetHexStr(v);
    ...
    case 'D': // Dpos/?v номер последнего убитого монстра в позиции
      if(Num<2){ MError("\"!!BU:D\"-wrong syntax."); RETURN(0) }
      if(Apply(&v,4,Mp,0)) break;
      bh=(char *)GetHexStr(v);
...
//        if((bh[0x10]&2)==0){ // не припятствие
    ...
    case 'O': // Opos/?v есть припятствие или нет
      if(Num<2){ MError("\"!!BU:O\"-wrong syntax."); RETURN(0) }
      if(Apply(&v,4,Mp,0)) break;
      bh=(char *)GetHexStr(v);
...
*/
    ...
    case 'S': // Stype/num/pos/side/slotnum/redraw вызвать существ на поле
      if(Num<6){ MError("\"!!BU:S\"-wrong syntax."); RETURN(0) }
...
int ERM_BattleField(char Cmd,int Num,_ToDo_*,Mes *Mp)
{
  ...
  switch(Cmd){
    ...
    case 'O': // Oposition/object - поместить
      if(Num<2){ MError("\"!!BF:O\"-wrong syntax."); RETURN(0) }
...
//        if(bh[0x1C]!=0) break; // есть мертвый стэк
    ...
    case 'M': // M#/#/#/#/#/#/#/#/#/#/#(/#) - биты доступности
      if(Num<11){ MError("\"!!BF:M\"-wrong syntax."); RETURN(0) }
...
int ERM_Battle(char Cmd,int Num,_ToDo_*,Mes *Mp)
{
  ...
  switch(Cmd){
    case 'H': // H0...1/$ - герой
      if(Num<2){ MError("\"!!BA:H\"-insufficient parameters."); RETURN(0) }
      ind=Mp->n[0];
...
//      Apply(&val,4,Mp,1);
    ...
    case 'M': // M0..1/0...6/type/number - монстры
      if(Num<4){ MError("\"!!BA:M\"-insufficient parameters."); RETURN(0) }
...
//      }
    ...
    case 'P': // Px/y/l - позиция
      if(Num<3){ MError("\"!!BA:P\"-insufficient parameters."); RETURN(0) }
    ...
    case 'O': // O?ownerA/?ownerD хозяева атаки и защиты
      if(Num<2){ MError("\"!!BA:O\"-insufficient parameters."); RETURN(0) }
      hp=G2B_HrA;
    ...
    case 'D': // D# - запретить=1/разрешить=0 битву
      if(Num<1){ MError("\"!!BA:D\"-insufficient parameters."); RETURN(0) }
      if(Mp->n[0]!=0) G2B_EnableBattle=0; else G2B_EnableBattle=1;
      break;
    case 'A': // A?# - Полностью автобитва или нет
      if(Num<1){ MError("\"!!BA:A\"-insufficient parameters."); RETURN(0) }
      val=G2B_CompleteAI;
      Apply(&val,4,Mp,0);
      break;
    case 'B': // B# - установить бэкграунд
      if(Num<1){ MError("\"!!BA:B\"-wrong syntax."); RETURN(0) }
      v=Mp->n[0];
    ...
    case 'Q': // Q$ - 3.58 quick battle (1=yes/0=no)
      if(Num<1){ MError("\"!!BA:Q\"-insufficient parameters."); RETURN(0) }
      val=QuickBattle();
      if(Apply(&val,4,Mp,0)) break;
      SetQuickBattle(val);
      break;
    case 'S': // S?# - осада замка или нет
      if(Num<1){ MError("\"!!BA:S\"-insufficient parameters."); RETURN(0) }
      __asm{
...
*/
  if(ERMFlags[996]!=0){
    Message("This feature does not work in Human vs Human network baced battle",1);
    RETURNV
  }
  __asm{
    mov    eax,0x699420
...
//    PopUpRadioButtons.Caption="Choose an Attack type";
    ...
    PopUpRadioButtons.DlgLeft=-1000;
    PopUpRadioButtons.DlgTop=-1000;
...
/*
  SendCreatures(&len,&buf);
  if((last+len)>10000){ MError("Too many date to send (>10000)"); return; }
  *((int *)&S2D_Buf[last])=len; last+=sizeof(int);
  for(i=0;i<len;i++) S2D_Buf[last+i]=buf[i];
  last+=len;
*/
...
//      v=M2B_GetMonNum(MF_Mon);
    ...
    case 'W': // кто ударяет
      ...
      Apply(&v,4,Mp,0); 
      break;
    default: MError("\"!!MF:\"-wrong command."); RETURN(0)
  }
  RETURN(1)
}
...
void __stdcall FixShowBFPopUpMonInfo(Byte *Mon,int)
{
  ...
  #include "templ.h"
   ParMod=1;
   do{
     if(MonInfoDlgPopUpSetting) break;
     if(Mon==0) break;
...
void SendERMVars(int from,int to){
  ...
  if(to>9999) to=9999;
  if(from>to) return;
  int *buf=new int[to-from+3];
  for(int i=from,j=2;i<=to;i++,j++) buf[j]=ERMVar2[i];
  buf[0]=from;
  buf[1]=to;
  NetData nd=NetData(NM_VARS,Send2Player,buf,to-from+3);
  nd.SendTo();
}
...
void SendERMFlags(int from,int to){
  ...
  if(from>to) return;
  int *buf=new int[to-from+3];
  for(int i=from,j=2;i<=to;i++,j++) buf[j]=ERMFlags[i];
  buf[0]=from;
  buf[1]=to;
  NetData nd=NetData(NM_FLAGS,Send2Player,buf,to-from+3);
  nd.SendTo();
}
...
void SendERMWVars(int hind,int from,int to){
  ...
  if(to>199) to=199;
  if(from>to) return;
  int *buf=new int[to-from+4];
  for(int i=from,j=3;i<=to;i++,j++) buf[j]=ERMVarH[hind][i];
  buf[0]=hind;
  buf[1]=from;
  buf[2]=to;
  NetData nd=NetData(NM_WVARS,Send2Player,buf,to-from+4);
  nd.SendTo();
}
...
void SendCustomRequest(int ind,int *var,int num){
  int *buf=new int[num+2];
  for(int i=0;i<num;i++) buf[i+2]=var[i]; // x vars
  buf[0]=ind; // function index
  buf[1]=num; // number of x vars
  NetData nd=NetData(NM_CUSTOM,Send2Player,buf,num+2);
  nd.SendTo();
}
...
int ERM_NetworkService(char Cmd,int Num,_ToDo_* /*sp*/,Mes *Mp)
{
  ...
  switch(Cmd){
    case 'D': // set to whom send a command (-1 to all)
      ...
      break;
    case 'V':{ // send vars
      if(Num<2){ MError("\"!!IP:V\"-wrong syntax."); RETURN(0) }
      int strt,stop;
      ...
      break;}
    case 'W':{ // send w vars
      if(Num<3){ MError("\"!!IP:W\"-wrong syntax."); RETURN(0) }
      int hind,strt,stop;
      ...
      break;}
    case 'F':{ // send flags
      if(Num<2){ MError("\"!!IP:F\"-wrong syntax."); RETURN(0) }
      int strt,stop;
      if(Apply(&strt,4,Mp,0)) break;
      if(Apply(&stop,4,Mp,1)) break;
    ...
    case 'R': // sync random generator
     SyncRGSeed();
     break;
    default: MError("\"!!IP\"-wrong command."); RETURN(0)
  }
  RETURN(1)
}